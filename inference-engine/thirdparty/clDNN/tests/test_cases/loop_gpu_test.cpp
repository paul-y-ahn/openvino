...
TEST(loop_gpu, basic_concat)
{
    ...
    EXPECT_EQ(output_layout.size.batch[0], 1);
    EXPECT_EQ(output_layout.size.feature[0], 128);
    EXPECT_EQ(output_layout.size.spatial[0], 4);
    EXPECT_EQ(output_layout.size.spatial[1], 5);
    ...
    // value check
    auto output_ptr = output.pointer<float>();
    for (size_t i=0, iend = input_data.size() * actual_iterations; i<iend; ++i) {
        const size_t j = i % input_data.size();
        size_t current_iteration = i / input_data.size();
        ++current_iteration;
        EXPECT_FLOAT_EQ(output_ptr[i], input_data[j] + eltwise_operand[j] * current_iteration);
    }
}
