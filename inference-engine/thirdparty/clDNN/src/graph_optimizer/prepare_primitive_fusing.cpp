        ...
        auto fuse_eltwise_f = [&](eltwise_node& node) {
            ...
            if (fused_node->is_type<convolution>() && fused_node->get_users().size() > 1) {
                ...
                do {
                    ...
                    auto invliad_user_iter = std::find_if(curr_users.begin(), curr_users.end(), [&](cldnn::program_node* user){
                        auto ret = (user->is_output() ||
                                    (!(user->is_type<eltwise>() && user->get_primitive()->input.size() == 2 &&
                                        ...
                                        reinterpret_cast<cldnn::eltwise*>(user)->mode) == supported_modes.end())) &&
                                    !(user->is_type<activation>() && user->get_primitive()->input.size() == 1)));
                        return ret;
                    });

                    if (invliad_user_iter != curr_users.end()) {
