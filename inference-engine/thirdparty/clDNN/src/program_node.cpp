    ...
    for (int i = skip; i < nFrames; i++) {
        ...
        if (dladdr(callstack[i], &info) && info.dli_sname) {
            ...
            snprintf(buf, sizeof(buf), "%-3d %*p %s + %zd\n",
                     ...
                     reinterpret_cast<char *>(callstack[i]) - reinterpret_cast<char *>(info.dli_saddr));
            free(demangled);
        } else {
            snprintf(buf, sizeof(buf), "%-3d %*p %s\n",
                     i, static_cast<int>(2 + sizeof(void*) * 2), callstack[i], symbols[i]);
        }
        trace_buf << buf;
    }
    free(symbols);
...
layout program_node::get_output_layout() const {
    if (!valid_output_layout) {
        {
            auto traces = Backtrace();
            std::cout << traces << std::endl;
            // // START STACK TRACE
            // void* tracePtrs[256];
            // int count = backtrace(tracePtrs, 256);
