...
private:
    ...
    std::vector<loop::primitive_mapping> input_mappings;
    std::vector<loop::primitive_mapping> output_mappings;
...
public:
    typed_program_node(std::shared_ptr<primitive> prim, program_impl& prog) :
        ...
        input_mappings(this->get_primitive()->input_mappings),
        output_mappings(this->get_primitive()->output_mappings),
    ...
    static std::vector<const loop::primitive_mapping*> find_primitive_mappings(
            ...
            const std::vector<loop::primitive_mapping>& primitive_map) {
    ...
    const std::vector<loop::primitive_mapping>& get_input_mappings() const { return input_mappings; }
    const std::vector<loop::primitive_mapping>& get_output_mappings() const { return output_mappings; }

    void update_primitive_map(const primitive_id& prevID, const primitive_id& newID, bool external_id = true) {
        if (external_id) {
            for (auto& pm : input_mappings) {
                if (pm.external_id == prevID) {
                    pm.external_id = newID;
                }
            }
            for (auto& pm : output_mappings) {
                if (pm.external_id == prevID) {
                    pm.external_id = newID;
                }
            }
    ...
    void build_body_program() const {
        ...
        for (auto& back_edge : back_edges) {
            ...
            if (input_mapping == input_mappings.end()) {
...
public:
    ...
    struct sliced_memory_binding {
        sliced_memory_binding(const primitive_id& from_id,
        ...
        void copy_to_concatenated_mem() const {
            mem_lock<uint8_t> to_lock{ concatenated_mem };
            int output_offset = byte_size_initial_offset;
            for (const auto& from_mem : sliced_mem) {
                mem_lock<uint8_t> from_lock{ from_mem };
                uint8_t* src = from_lock.data();
                uint8_t* dst = to_lock.data() + output_offset;
                std::copy(src, src + byte_size_iteration_elements, dst);
                output_offset += byte_size_stride;
            }
        }

        memory_impl::ptr copy_and_get_sliced_mem(int iteration) const {
            const int offset = byte_size_initial_offset + byte_size_stride * iteration;
            {
                mem_lock<uint8_t> from_lock{ concatenated_mem };
                mem_lock<uint8_t> to_lock{ sliced_mem.at(iteration) };
                const auto src = from_lock.begin() + offset;
                const auto dst = to_lock.begin();
                std::copy(src, src + byte_size_iteration_elements, dst);
            }
            return sliced_mem.at(iteration);
        }
        ...
        const int byte_size_iteration_elements;
        const int byte_size_stride;
        const int byte_size_initial_offset;
    ...
    std::vector<sliced_memory_binding> input_iteration_mem;
    std::vector<sliced_memory_binding> output_iteration_mem;
