...
JitConstants FusedOpsCodeGenerator::MakeOpJitConstants(const FusedOpsConfiguration& conf,
    ...
    auto get_fused_input = [&](size_t index) -> std::string {
        auto input_name = GetOutputVarName(in_var, fused_op_ids[index].first);
    ...
    for (size_t i = 0; i < fused_op_ids.size(); i++) {
        input_vars.push_back(get_fused_input(i));
    }

    // If the number of input_vars are larger than max_num_input_vars, remove dst from input variable list.
