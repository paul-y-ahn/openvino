...
void LayerTestsCommon::ConfigureNetwork() {
    ...
    core->SetConfig({{CLDNN_CONFIG_KEY(SOURCES_DUMPS_DIR), "/home/ahnyoung/cldnn/openvino/bin/intel64/Debug/exec_graph_dump/kernels"}}, targetDevice);
}
...
static void printPerformanceCounts(const std::map<std::string, InferenceEngine::InferenceEngineProfileInfo>& performanceMap,
    ...
    std::cout << "performanceMap : " << performanceMap.size() << std::endl;
    std::cout << "performanceMapSorted : " << performanceMapSorted.size() << std::endl;

    for (const auto & it : performanceMapSorted) {
        std::string toPrint(it.first);
        const int maxLayerName = 30;
        // if (std::string(it.second.layer_type).find("ConvolutionBackprop") == std::string::npos) continue;
...
void LayerTestsCommon::Infer() {
    ...
    std::string exec_graph_path = "/home/ahnyoung/cldnn/openvino/bin/intel64/Debug/exec_graph_dump/graph.xml";
    execGraphInfo.serialize(exec_graph_path);
    std::cout << "Now :: dumping exec graph to " << exec_graph_path << std::endl;
